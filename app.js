...
function renderNotes() {
  ...
  if (currentFilter && currentFilter !== "All") {
    filteredNotes = notes.filter(
      (note) => note.tags && note.tags.includes(currentFilter),
    );
  }
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (tagInput) {
    ...
    tagInput.addEventListener("input", () => {
      ...
      tagSuggestions.innerHTML = matches.length
        ? matches
            .map((tag) => `<li onclick="selectTag('${tag}')">${tag}</li>`)
            .join("")
        : "";
    });
  ...
  if (menuBtn && panel) {
    document.body.appendChild(panel); // moves panel out of header
    const openMenu = () => {
    ...
    // position panel under the icon on resize
    const positionPanel = () => {
